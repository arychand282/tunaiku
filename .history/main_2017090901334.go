...
func main() {
	...
	router.GET("/bestprofits", func(c *gin.Context) {
		...
		var newStruct DataProfit
		var newStructs []DataProfit

		for i := 0; i < len(dataprofits)-1; i++ {
			newStruct.DateSell = dataprofits[i].Date_Data
			newStruct.Profit = dataprofits[i].Open
			...
			newStruct.DateBuy = dataprofits[i].Date_Data
