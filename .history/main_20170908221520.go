...
func main() {
	...
	router.GET("/bestprofits", func(c *gin.Context) {
		...
		for i := 0; i < len(dataprofits)-1; i++ {
			...
			type arrayrangesperrow struct {}
			for j := i + 1; j < len(dataprofits); j++ {
				if dataprofits[j].Date_Data != "" {
					temp := dataprofits[j].Date_Data
					type arrayrangesperrow.temp := {
						profit = dataprofits[j].Close - dataprofits[i].Open
					}					
					arrayrangesperrow.temp.profit = dataprofits[j].Close - dataprofits[i].Open

				}
				//rangesperrow.profit := dataprofits[j].Close - dataprofits[i].Open
				dataprofits[j].Range_Value = dataprofits[j].Close - dataprofits[i].Open

				arrayrangesperrow = append(arrayrangesperrow, rangesperrow)
			}
