...
func main() {
	...
	router.POST("/nominalamount", func(c *gin.Context) {
		...
		for {
			if totalintnumbe > 15000000 {
				break
			}
			interestrate := testnumb * 3 / 100
			testnumbwithinterest := testnumb + interestrate
			totalintnumbe = totalintnumbe + testnumbwithinterest

			fmt.Println(totalintnumbe)
