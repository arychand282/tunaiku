...
func main() {
	...
	router.POST("/evenoddnumber", func(c *gin.Context) {
		...
		totalnumber := strconv.ParseInt(evennumber) + strconv.ParseInt(oddnumber)
		...
		// Fastest way to append strings
		buffer.WriteString("even number: " + evennumber + ", ")
		buffer.WriteString("odd number: " + oddnumber + ", ")
		buffer.WriteString("total: " + totalnumber)
		defer stmt.Close()
		name := buffer.String()
		c.JSON(http.StatusOK, gin.H{
			"message": fmt.Sprintf(" %s successfully created", name),
