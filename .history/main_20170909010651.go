...
func main() {
	...
	router.GET("/bestprofits", func(c *gin.Context) {
		...
		for rows.Next() {
			err = rows.Scan(&data.Date_Data, &data.Open, &data.High, &data.Low, &data.Close, &dataprofit.Adj_Close, &dataprofit.Volume)
		...
		for i := 0; i < len(dataprofits)-1; i++ {
			...
			for j := i + 1; j < len(dataprofits); j++ {
				...
				if tempProfit > dataProfit.Profit {
					dataProfit.DateSell = dataprofits[j].Date_Data
					dataProfit.Profit = tempProfit
				}
			}

			dataProfit.DateBuy = dataprofits[i].Date_Data
