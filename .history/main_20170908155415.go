...
func main() {
	...
	router.GET("/bestprofits", func(c *gin.Context) {
		...
		for i := 0; i < len(dataprofits); i++ {
			...
			fmt.Println("maximum profit buy day %s : ", dataprofits[i].Date_Data)
			_, max := minmaxProfit(arrayranges)
			fmt.Print(max)
			fmt.Println("by index Close: %")
		}
...
func minmaxProfit(array []int) (int, int) {
	var max int = array[0]
	var min int = array[0]
