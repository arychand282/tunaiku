...
func main() {
	...
	router.POST("/nominalamount", func(c *gin.Context) {
		var buffer bytes.Buffer
		nominalamount, err := strconv.Atoi(c.PostForm("nominal_amount"))
		if err != nil {
			fmt.Println(err.Error())
		}
		...
		for {
			interestrate := float64(nominalamount * 3 / 100)
			total := total + float64(nominalamount) + interestrate
		...
		successmess := buffer.String()
		c.JSON(http.StatusOK, gin.H{
			"message": fmt.Sprintf("interation successfully created", successmess),
		})
	})
