...
func main() {
	...
	router.GET("/bestprofits", func(c *gin.Context) {
		...
		for i := 0; i < len(dataprofits)-1; i++ {
			// var arrayrangesperrow []int

			//var tempStruct TempStruct
			//var tempStructs []TempStruct

			newStruct.newMaxest.DateName = ""
			newStruct.newMaxest.Profit = 0
			for j := i + 1; j < len(dataprofits); j++ {

				// tempStruct.DateName = dataprofits[j].Date_Data
				// tempStruct.Profit = dataprofits[j].Close - dataprofits[i].Open

				// tempStructs = append(tempStructs, tempStruct)

				tempProfit := dataprofits[j].Close - dataprofits[i].Open

				if tempProfit > newStruct.newMaxest.Profit {
					newStruct.newMaxest.DateName = dataprofits[j].Date_Data
					newStruct.newMaxest.Profit = tempProfit
				}

			}
			//_, max, _, _ := minmaxProfit(arrayrangesperrow)
			//fmt.Println("\nmaximum profit buy day ", dataprofits[i].Date_Data, ": ", max)

			//maxprofitallrows = append(maxprofitallrows, max)
			newStruct.DateName = dataprofits[i].Date_Data
			//newStruct.TempStructArray = tempStructs

			newStructs = append(newStructs, newStruct)
		}
		fmt.Println(newStructs)

		//fmt.Println(maxprofitallrows)
		//_, maximumprofit, _, _ := minmaxProfit(maxprofitallrows)
